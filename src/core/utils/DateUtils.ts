...
export const dateOrUndefined = (date?: Date | string) => {
  if (!date) {
    return undefined;
  }
...
export const parseDate = (context?: string) => {
  if (!context) {
    return undefined;
  }
...
export const filterDates = (dates: (Date | string | null | undefined)[], filterOutFutureDatesBy = '1d') => {
...
export const sortDates = (...dates: (Date | string | null | undefined)[]) => {
  return filterDates(dates).sort(DateSorter);
};

export const minDate = (...dates: (Date | string | null | undefined)[]) => {
...
export const maxDate = (...dates: (Date | string | null | undefined)[]) => {
  const sortedDates = sortDates(...dates);
  if (sortedDates.length === 0) {
    return undefined;
  }
...
export const durationToParts = (duration?: DurationString | number) => {
  if (!duration) {
    return {
      days: 0, hours: 0, minutes: 0, seconds: 0,
    };
  }
  const msDuration = typeof duration === 'number' ? duration : ms(duration);
  const days = msDuration >= 864e5 ? Math.floor(msDuration / 864e5) : 0;
  const hours = msDuration - (days * 864e5) >= 36e5 ? Math.floor((msDuration - (days * 864e5)) / 36e5) : 0;
  const minutes = msDuration - (days * 864e5) - (hours * 36e5) >= 6e4 ? Math.floor((msDuration - (days * 864e5) - (hours * 36e5)) / 6e4) : 0;
  const seconds = msDuration - (days * 864e5) - (hours * 36e5) - (minutes * 6e4) >= 1e3 ? Math.floor((msDuration - (days * 864e5) - (hours * 36e5) - (minutes * 6e4)) / 1e3) : 0;
  return {
    days, hours, minutes, seconds, 
  };
};

export const durationToDurationString = (duration?: DurationString | number) => {
  if (!duration) {
    return '0d 0h 0m 0s';
  }
  const {
    days, hours, minutes, seconds, 
  } = durationToParts(duration);
  return `${days}d ${hours}h ${minutes}m ${seconds}s`;
};

export const timeAgo = (date: Date, { from = Date.now(), defaultValue = 'just now' }: TimeAgoOptions = {}) => {
...
export type TimeUnit = 'day' | 'hour' | 'minute' | 'month' | 'second' | 'year';

export type DateParsingOptions = {
  include?: TimeUnit[];
};

export const durationToHoursAndMinutes = (
  duration: DurationString | number 
) => {
  const msDuration = typeof duration === 'number' ? duration : ms(duration);
  const hours = Math.floor(msDuration / 36e5);
  const minutes = Math.floor((msDuration % 36e5) / 6e4);
  return { hours, minutes };
};

export const calcTimeUntil = (until: number, now = Date.now()) => {
  const hours = Math.floor(Math.abs(until - now) / 36e5);
  const minutes = Math.floor(Math.abs((until - now) % 36e5) / 6e4);
  const seconds = Math.floor(Math.abs((until - now) % 6e4) / 1e3);
  return [hours, minutes, seconds].map((p) => p.toFixed(0).padStart(2, '0')).join(':');
};
